...
const COLUMN_ALIASES: Record<string, keyof BacklogItem | null> = {
  id: 'id',
  status: 'status',
  state: 'status',
  ownerzone: 'ownerZone',
...
type CandidateProject = {
  ...
  rootPath: string;
  projectPath: string;
  primaryBacklogPath: string;
  backlogSources: BacklogSource[];
...
type WatchSession = {
  ...
  unavailableRoots: Set<string>;
  erroredRoots: Set<string>;
};
...
function parseProjectRoots(rawRoots: unknown): string[] {
...
function getOrCreateWatchSession(roots: string[]): WatchSession {
  ...
  const session: WatchSession = {
    ...
    unavailableRoots: new Set<string>(),
    erroredRoots: new Set<string>()
  };
...
function notifyWatchSession(key: string, payload: Omit<WatchEventPayload, 'watchStatus'>) {
...
function closeWatchSession(key: string) {
  const session = watchSessions.get(key);
  if (!session) {
    return;
  }
...
function createRootsKey(roots: string[]): string {
  return roots.map((root) => root.toLowerCase()).sort().join('|');
}

function getWatchStatus(session: WatchSession): WatchStatus {
  return {
    roots: [...session.roots],
...
function isRelevantWatchedPath(relativePath: string): boolean {
  if (!relativePath) {
    return true;
  }
...
async function loadProjects(roots: string[]): Promise<ProjectSummary[]> {
...
async function findProjectById(projectId: string, roots: string[]): Promise<CandidateProject | null> {
  const candidates = await discoverProjects(roots);
  return candidates.find((candidate) => candidate.id === projectId) ?? null;
}

async function loadProjectDetail(project: CandidateProject): Promise<ProjectDetail> {
  ...
  return {
    ...
    backlogZones: uniqueSorted(backlogItems.map((item) => item.ownerZone).filter(Boolean) as string[]),
    logZones: uniqueSorted(logs.map((entry) => entry.zone).filter(Boolean) as string[])
  };
}

async function discoverProjects(roots: string[]): Promise<CandidateProject[]> {
  const allProjects = await Promise.all(
    roots.map(async (rootPath) => {
      try {
        ...
        return candidates.filter(Boolean) as CandidateProject[];
      } catch {
        return [];
      }
    })
...
function buildProjectSummary(
  ...
  const attention =
    blockedCount > 0
      ? { tone: 'blocked' as const, label: `${blockedCount} blocked item${blockedCount === 1 ? '' : 's'}` }
      : inProgressCount > 0
        ? { tone: 'active' as const, label: `${inProgressCount} active item${inProgressCount === 1 ? '' : 's'}` }
        : { tone: 'quiet' as const, label: 'No active blockers' };
...
async function determineLatestChangedAt(project: CandidateProject): Promise<string | null> {
  ...
  const latest = stats.reduce<Date | null>((current, value) => {
    if (!value) {
      return current;
    }
...
async function parseAllBacklogs(project: CandidateProject): Promise<BacklogItem[]> {
  const parsed = await Promise.all(
    project.backlogSources.map(async (source) => parseBacklog(source.path, source.key, source.label))
  );
  return parsed.flat();
}
...
async function parseBacklog(
  ...
  return dataLines
    .map((line) =>
      parseBacklogRow(header, splitMarkdownRow(line), {
        backlogSourceKey,
        backlogSourceLabel
      })
    )
...
function parseBacklogRow(
  headers: string[],
  values: string[],
  source: { backlogSourceKey: string; backlogSourceLabel: string }
): BacklogItem | null {
...
function normalizeStatus(value: string): BacklogStatus {
...
async function loadProjectLogs(logsPath: string): Promise<ProjectLogEntry[]> {
...
async function parseLogFile(filePath: string): Promise<ProjectLogEntry[]> {
  const content = await fs.readFile(filePath, 'utf8');
  const lines = content.split(/\r?\n/).filter(Boolean);
  const parsed: ProjectLogEntry[] = [];
...
async function loadFrameworkFiles(project: CandidateProject): Promise<FrameworkFileMeta[]> {
  const baseFiles = await Promise.all(
    FRAMEWORK_FILE_CONFIG.map(async (config) => {
  ...
  return [...(baseFiles.filter(Boolean) as FrameworkFileMeta[]), ...backlogFiles];
}

async function loadFrameworkFile(project: CandidateProject, key: FrameworkFileKey): Promise<FileContent | null> {
...
async function findBacklogPath(projectPath: string): Promise<string | null> {
...
async function collectBacklogSources(
  projectPath: string,
  primaryBacklogPath: string
...
async function collectNamedFiles(rootPath: string, targetName: string): Promise<string[]> {
  const entries = await fs.readdir(rootPath, { withFileTypes: true });
  const nested = await Promise.all(
    entries.map(async (entry) => {
      const fullPath = path.join(rootPath, entry.name);
...
async function collectNamedMarkdownFiles(rootPath: string): Promise<string[]> {
  const entries = await fs.readdir(rootPath, { withFileTypes: true });
  const nested = await Promise.all(
    entries.map(async (entry) => {
      const fullPath = path.join(rootPath, entry.name);
...
function createBacklogSourceKey(absolutePath: string): string {
  return `backlog:${Buffer.from(absolutePath).toString('base64url')}`;
}

function toProjectRelativeLabel(projectPath: string, absolutePath: string): string {
...
function createEmptyCounts(): Record<BacklogStatus, number> {
  return {
    todo: 0,
    in_progress: 0,
    blocked: 0,
